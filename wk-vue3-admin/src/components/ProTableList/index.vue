...
const handleSelectionChange = (val: any) => {
    ...
    nextTick(() => {
        const list = tableData.value.map((el: any) => el[props.rowKey])
        remove(selectRowsNew as any[], (el: any) => {
            return list.includes(el[props.rowKey])
        })
        emits(
            "update:selectRows",
            uniqBy(
                concat(selectRowsNew, JSON.parse(JSON.stringify(val))),
                (el: any) => el[props.rowKey]
            )
        )
    })
}
...
const getList = () => {
    ...
    request({
        url: props.url,
        method: props.method as any, // 这里加上类型断言，解决类型不匹配问题
