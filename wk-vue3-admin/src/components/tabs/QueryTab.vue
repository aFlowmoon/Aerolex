  ...
  <div class="search-block">
    ...
    <div class="search-box">
      <div class="input-action-wrapper-vertical" style="position: relative;">
        <textarea class="search-input-area" v-model="keyword" placeholder="请输入你想检索的信息"></textarea>
        <!-- 左下角法规检索按钮 -->
        <div class="left-action-btn" style="display: flex;">
          <button class="main-action-btn" @click="onSearch">
            <img src="@/assets/images/main-search-law.svg" class="main-btn-img" alt="法规检索">
            <span class="main-btn-text">法规检索</span>
          </button>
        </div>
        <!-- 右下角按钮组 -->
        <div class="input-action-row">
          <div class="input-action-btns">
            <label for="query-file-upload" class="icon-btn-plain" title="上传附件">
              <img src="@/assets/images/attachment.svg" alt="上传附件" class="icon-img">
            </label>
            <input id="query-file-upload" type="file" style="display:none;" accept=".pdf,.doc,.docx,.txt" @change="onFileChange">
...
function onSearch() {
  ...
  emit('addChatMessage', keyword.value, true, 'query')
  keyword.value = ''
}
function onSend() {
  onSearch()
