<template>
  <div class="search-block">
    <div class="search-row">
      <label class="search-label">条款号</label>
      <select class="search-select" v-model="article">
        <option value="">请选择</option>
        <option value="A001">A001</option>
        <option value="A002">A002</option>
      </select>
      <label class="search-label">文件号</label>
      <select class="search-select" v-model="fileNo">
        <option value="">请选择</option>
        <option value="F001">F001</option>
        <option value="F002">F002</option>
      </select>
      <label class="search-label">章节号</label>
      <select class="search-select" v-model="chapter">
        <option value="">请选择</option>
    ...
    <div class="search-box">
      <div class="input-action-wrapper-vertical" style="position: relative;">
        <textarea class="search-input-area" v-model="keyword" placeholder="请输入你想比对的信息"></textarea>
        <!-- 左下角法规对比按钮 -->
        <div class="left-action-btn" style="display: flex;">
          <button class="main-action-btn" @click="onCompare">
            <img src="@/assets/images/main-compare-law.svg" class="main-btn-img" alt="法规对比">
            <span class="main-btn-text">法规对比</span>
          </button>
        </div>
        <!-- 右下角按钮组 -->
        <div class="input-action-row">
          <div class="input-action-btns">
            <label for="compare-file-upload" class="icon-btn-plain" title="上传附件">
              <img src="@/assets/images/attachment.svg" alt="上传附件" class="icon-img">
            </label>
            <input id="compare-file-upload" type="file" style="display:none;" accept=".pdf,.doc,.docx,.txt" @change="onFileChange">
...
function onCompare() {
  ...
  emit('addChatMessage', keyword.value, true, 'compare')
  keyword.value = ''
}
function onSend() {
  onCompare()
